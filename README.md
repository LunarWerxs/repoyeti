...
- 🌿 &nbsp;**Git-graph history** — the commit graph with lanes and merges, lazily paged.
...
- **AI agents (MCP):** `repoyeti mcp` exposes 14 tools over the same daemon; the full HTTP surface is at `GET /api/openapi.json`.
